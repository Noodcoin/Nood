...
#if defined(NDEBUG)
#error "nood cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "nood Signed Message:\n";
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
	...
	if (nHeight >= 1) {
		...
		if (mNodeCoins == 0) {
			...
			LogPrintf("[nood] Adjusting nood at height %d with %d masternodes (%d % locked Nodex) and %d Nodex supply at %ld\n", nHeight, nMasternodeCount, lockedCoinValue*100, nMoneySupply, GetTime());
			LogPrintf("[nood] Masternode: %d\n", masternodeMultiplier*100);
			LogPrintf("[nood] Staker: %d\n", (1 - masternodeMultiplier)*100);
