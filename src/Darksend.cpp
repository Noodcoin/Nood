    ...
    if (fMasterNode) {
        ...
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat nood and make it unusable. To
                stop these kinds of attacks 1 in 10 successful transactions are charged. This
                adds up to a cost of 0.001 nood per transaction on average.
            */
            if (r <= 10) {
...
void CDarksendPool::GetDenominationsToString(int nDenom, std::string& strDenom)
{
    ...
    //
    // bit 0 - 100 nood + 1 (bit on if present)
    // bit 1 - 10 nood + 1
    // bit 2 - 1 nood + 1
    // bit 3 - .1 nood + 1
    // bit 3 - non-denom
