...
*Setup instructions for a gitian build of nood using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the nood
...
Clone the git repositories for nood and gitian and then checkout the nood version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/noodcoin/nood-core.git
cd nood-core
...
Follow the instructions in [doc/release-process.md](release-process.md) in the nood-core repository
...
To build nood (for Linux, OSX, and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the nood-core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/nood-core/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/noodcoin/nood-core
...
./bin/gbuild --commit nood=${COMMIT} --url nood=${URL} ../nood/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit nood=${COMMIT} --url nood=${URL} ../nood/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit nood=${COMMIT} --url nood=${URL} ../nood/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/nood-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/nood-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/nood-build.assert
```
...
[noodcoin/gitian.sigs](https://github.com/noodcoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
